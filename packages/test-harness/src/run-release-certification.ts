...
try {
  ...
  const authorityRows = Object.entries(report.authorities)
    .map(
      ([name, value]) =>
        `<tr><td><code>${html(name)}</code></td><td class="${value === 'unavailable' ? 'unavailable' : 'passed'}">${value === 'unavailable' ? '— unavailable' : `✓ ${html(value)}`}</td></tr>`,
    )
    .join('')
  const authoritySummary =
    productionReviewer === undefined
      ? 'Conditional evidence: the deterministic release journey passed; authenticated provider and GitHub release authority remain unavailable.'
      : 'Conditional evidence: both authenticated provider journeys passed; GitHub release authority remains unavailable.'
  await mkdir(reportRoot, { recursive: true })
  await writeFile(join(reportRoot, 'report.json'), `${JSON.stringify(report, null, 2)}\n`)
  await writeFile(
    join(reportRoot, 'index.html'),
    `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>Factory release evidence</title><style>body{font:16px/1.5 system-ui;max-width:900px;margin:3rem auto;padding:0 1rem;color:#171a1c}h1{margin-bottom:.25rem}.verdict{display:inline-block;margin:.35rem 0 .8rem;padding:.25rem .55rem;background:#8a4300;color:white;border-radius:4px;font-weight:800;letter-spacing:.04em}.summary{border-left:4px solid #b76800;background:#fff6e8;padding:.8rem 1rem;margin-top:0}h2{margin-top:2rem}code{overflow-wrap:anywhere;color:#36434a}table{border-collapse:collapse;width:100%;table-layout:auto}th{background:#e9eef0}th,td{border:1px solid #aab6bb;padding:.6rem;text-align:left;vertical-align:top}.passed{color:#176b35}.unavailable{color:#8a4300;font-weight:600}@media(max-width:650px){body{margin:1.5rem auto;font-size:15px}table{display:block;overflow-x:auto}}</style></head><body><h1>Factory release evidence</h1><p class="verdict">NOT RELEASE-CERTIFIED</p><p class="summary"><strong>${html(authoritySummary)}</strong></p><p>Artifact <code>${html(release.executableSha256)}</code></p><h2>Authority</h2><table class="authority"><thead><tr><th>Capability</th><th>Evidence</th></tr></thead><tbody>${authorityRows}</tbody></table><h2>Passed deterministic journeys</h2><table><thead><tr><th>Journey</th><th>Status</th><th>Evidence</th></tr></thead><tbody>${journeys.map(item => `<tr><td>${html(item.name)}</td><td class="passed">✓ ${item.status}</td><td>${html(item.detail)}</td></tr>`).join('')}</tbody></table></body></html>`,
  )
